...
zoo_test(){
    ...
    zoo_ns="nirmata" # Set the namespace explicitly
    zoos=$(kubectl get pod -n $zoo_ns -l 'nirmata.io/service.name in (zookeeper, zk)' --no-headers | awk '{print $1}')
    zoo_num=0
    zoo_leader=""
    for zoo in $zoos; do
        echo "Checking pod: $zoo"
        curr_zoo=$(kubectl -n $zoo_ns exec $zoo -- sh -c "/apache-zookeeper-3.9.2-bin/bin/zkServer.sh status" 2>&1)
        echo "zkServer.sh output for $zoo:"
        echo "$curr_zoo"
        ...
        if [[ $curr_zoo =~ "leader" ]];then
            ...
            echo "Warning: $zoo is zookeeper standalone!"
            zoo_leader="$zoo_leader $zoo"
        else
            echo "Error: $zoo appears to have failed!! (not follower/leader/standalone)"
    ...
    if [ -z "$zoo_leader" ];then
        echo "Error: No Zookeeper Leader found!!"
        zoo_error=1
    fi
    [ $zoo_error -eq 0 ] && good "Success: Zookeeper passed tests"
}
